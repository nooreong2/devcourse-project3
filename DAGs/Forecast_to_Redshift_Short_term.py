...
# 카테고리명 매핑
category_mapping = {"POP": "강수확률", "T1H": "기온", "RN1": "강수량", "WSD": "풍속"}
...
def fetch_and_store_weather_data():
    ...
    for stadium in stadiums:
        ...
        # 강수 확률 데이터 가져오기
        weather_data_rain_prob = get_weather_data_rain_prob(
            auth_key, base_date, base_time_rain_prob, nx, ny, page_no, num_of_rows, data_type
        )
...
def save_to_redshift(df, table_name, stadium_name):
    ...
    # 테이블이 존재하는지 확인
    cur.execute(
        f"SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = '{schema}' AND table_name = '{table_name}_{stadium_name}');"
    )
